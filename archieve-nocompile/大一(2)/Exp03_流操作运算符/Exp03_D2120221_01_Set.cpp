...
int set::getElem(int a) const
{
    return elements[a];
}

int set::getNum() const
{
    return num;
}
...
{
    //写文件
    ofstream ofile1("./d1.txt");
    char writeinfo1[] = "23   17   56   67   88   33   94   45   13\n22   18   25   66   86   99   35";
    ofile1 << writeinfo1;
    ofile1.close();

    ofstream ofile2("./d2.txt");
    char writeinfo2[] = "32   21   33   56   66   85   99   20   11\n45   98   22   67   81   91   69   65   76   89";
    ofile2 << writeinfo2;
    ofile2.close();
    ...
    //读文件
    ifstream ifileobj1("./d1.txt");
    while (!ifileobj1.eof())
    {
        ifileobj1 >> tmp_num;
        contain1.AddElem(tmp_num);
    }
    ifileobj1.close();


    ifstream ifileobj2("./d2.txt");
    while (!ifileobj2.eof())
    {
        ifileobj2 >> tmp_num;
        contain2.AddElem(tmp_num);
    }
    ifileobj2.close();

    set ret_set = contain1.Intersect(contain2);

    //debug
    ret_set.print();

    //将交集写出（us-ascii）
    ofstream ofile3("./Exp03_D2120221_03_set.txt");
    for (int i = 0; i < ret_set.getNum(); i++)
    {
        ofile3 << ret_set.getElem(i) << ' ';
        if (i % 4 == 3)
        {
            ofile3 << endl;
        }
