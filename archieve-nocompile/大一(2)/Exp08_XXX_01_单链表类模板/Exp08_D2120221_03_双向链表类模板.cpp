...
template <typename T>
void DblList<T>::InsertRear(const T &data)
{
    //新节点在链尾
...
template <typename T>
DblNode<T> *DblList<T>::Find(T data)
{
    current = head->rlink;
...
template <typename T>
DblList<T> &DblList<T>::merge(DblList<T> &nd)
{
    current = head->llink;
    nd.current = nd.head->rlink;

    while (nd.current != nd.head)
    {
        current = head->llink;
        InsertRear(nd.current->info);
        nd.current = nd.current->rlink;
    }
    return *this;
}
...
int main()
{
    ...
    tp = Dbl1.Find(7);
    if (tp)
        cout << "查到的结点数据为：" << tp->Getinfo() << " 清除" << endl;
    ...
    cout << "\n\n";
    int b[11] = {19, 11, 17, 13, 7, 2, 3, 5, 23, 31, 29};
    for (i = 0; i < 11; i++)
        Dbl2.InsertRear(b[i]);
    Dbl2.Print();

    cout << "\n\n";
    int c[11] = {11, 11, 11, 13, 7, 2, 3, 5, 23, 31, 29};
    for (i = 0; i < 11; i++)
        Dbl3.InsertRear(c[i]);
    Dbl3.Print();
    cout << "\n\n";
